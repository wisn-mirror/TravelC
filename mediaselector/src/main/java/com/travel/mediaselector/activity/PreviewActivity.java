...
import com.travel.library.view.widget.MViewPager;
import com.travel.mediaselector.ImageSelector;
import com.travel.mediaselector.R;
import com.travel.mediaselector.adapter.ImagePagerAdapter;
...
public class PreviewActivity extends AppCompatActivity {
    ...
    // 用Intent传输发生的错误问题。
    private static ArrayList<MediaInfo> tempImages;
    private static ArrayList<MediaInfo> tempSelectImages;

    private ArrayList<MediaInfo> mImages;
    private ArrayList<MediaInfo> mSelectImages;
    ...
    private void initView() {
        vpImage = (MViewPager) findViewById(R.id.vp_image);
    ...
    private void initViewPager() {
        ...
        adapter.setOnItemClickListener(new ImagePagerAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(int position, MediaInfo image) {
                if (isShowBar) {
                    hideBar();
                } else {
                    showBar();
    ...
    private void hideBar() {
        ...
        ObjectAnimator animator = ObjectAnimator.ofFloat(rlTopBar, "translationY",
                0, -rlTopBar.getHeight()).setDuration(200);
        animator.addListener(new AnimatorListenerAdapter() {
            @Override
    ...
    private void clickSelect() {
        ...
        if (mImages != null && mImages.size() > position) {
            MediaInfo image = mImages.get(position);
    ...
    private void changeSelect(MediaInfo image) {
