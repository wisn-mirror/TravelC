...
public final class CameraManager {
     ...
     * 双击放大
     */
    public synchronized void doubleTagHandleZoom() {
     ...
     * @param event
     */
    public synchronized void handleFocusMetering(MotionEvent event) {
        try {
            if (camera == null || event == null) return;
            Camera.Parameters params = camera.getParameters();
...
//                Log.i("Camera","进入放大方法zoom="+zoom);
                zoom = zoom + 2;
            } else if (zoom > 2) {
//                Log.i("Camera","进入缩小方法zoom="+zoom);
                zoom = zoom - 2;
            }
            params.setZoom(zoom);
            camera.setParameters(params);
