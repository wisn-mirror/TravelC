...
public class BlurBehindHelper {
    ...
    private static BlurBehindHelper mInstance;

    public static BlurBehindHelper getInstance() {
        if (mInstance == null) {
            mInstance = new BlurBehindHelper();
        }
        return mInstance;
    }
    ...
    public BlurBehindHelper withAlpha(int alpha) {
        this.mAlpha = alpha;
        return this;
    }

    public BlurBehindHelper withFilterColor(int filterColor) {
    ...
    private class CacheBlurBehindAndExecuteTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            Bitmap blurredBitmap = ImageUtils.apply(activity, image, CONSTANT_BLUR_RADIUS);
     ...
     */
    public static interface OnBlurCompleteListener {
