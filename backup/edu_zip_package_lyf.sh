...
backup_Dir=/Users/mac/Desktop/Version/takeoutBusinessReleasebackup
...
echo 开始打包
sh ./gradlew clean assembleEdu | grep -v 'takeoutBusinessEdubuildlog.txt' > ${BuildLog}/takeoutBusinessEdubuildlog.txt
...
echo 'BUILDFAILED'
cat ${BuildLog}/takeoutBusinessEdubuildlog.txt
exit
fi
done <${BuildLog}/takeoutBusinessEdubuildlog.txt
...
#加固
echo 开始加固
ls ${WORKSPACE}/build/outputs/apk/edu/ |grep -v 'apkjiagulist.txt' > ${WORKSPACE}/build/apkjiagulist.txt
while read lineapk
do
echo ${WORKSPACE}/build/outputs/apk/${lineapk}
java -jar ${jgPath}/jiagu.jar -jiagu ${WORKSPACE}/build/outputs/apk/edu/${lineapk} ${WORKSPACE}/build/outputs/jiagu/ -autosign
